...
export default function MovingAvatar({ 
  ...
  // Change target periodically depending on status.
  useEffect(() => {
    const getNewTarget = () => {
      let attempts = 0;
      let newPos: Vector3;
    ...
    // Idle: move more often
    // Working: move less often
    // Thinking: move rarely
    // Error: stay mostly still
    const getInterval = () => {
      switch (state.status) {
        case 'idle':
